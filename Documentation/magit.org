...
their arguments to the default value when the transient prefix command
is invoked again.
...
arguments are reset to the default.  Not cool; so Magit do not do that
by default.
...
- User Option: magit-prefix-use-buffer-arguments
  ...
  are currenly in effect in the buffer that their suffixes update.

  The ~magit-diff~, ~magit-log~ and ~magit-show-refs~ transient prefix
  commands are affected by this option.  (Note that ~magit-show-refs~
  only acts as a prefix if invoked with a prefix argument.  Otherwise
...
- When ~magit-show-commit~ is invoked directly from a log buffer, then
  ...
  buffer and not from the diff default arguments.  (Also, it would be
  a bad idea anyway to add a file filter to the default value.)

- Even though the are suffixes of the diff prefix ~magit-show-commit~
  and ~magit-stash-show~ do not use the diff buffer used by the other
  diff commands.  The use the dedicated revision and stash buffers.
  But at the time you invoke the diff prefix it is unknown to Magit
  which of the suffix commands you are going to invoke.  While not
  certain more often than not users invoke one of the commands that
  use the diff buffer, so the initial infix arguments are those used
  in that buffer.  However if you invoke one of these commands
  directly, then Magit can always use the arguments from the correct
  buffer.

- The log prefix also features reflog commands, but these commands do
  not use the log arguments from the log prefix.  This still needs
  some work.
